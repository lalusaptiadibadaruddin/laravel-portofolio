...
<div class="card">
                ...
                <div class="row my-0">
                    <div class="col-lg-12">
                        <button class="btn btn-secondary rounded mb-2" data-bs-toggle="modal" data-bs-target="#ModalAdd"><i
                                class="bi bi-plus"></i> Add Interest</button>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="titleModalLabel">Add Interest</h5>
                ...
                <div class="modal-body">
                    <form class="row g-3" id="form-add" enctype="multipart/form-data" action="#" method="POST">
                        ...
                        <div class="form-group pb-2">
                            <label for="name" class="pb-1">Name Interest</label>
                            <input type="text" name="name" id="name" class="form-control"
                                placeholder="name" required />
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-success" id="add_interest_btn">Submit</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <div class="modal fade" id="ModalEdit" tabindex="-1" aria-labelledby="titleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="titleModalLabel">Edit Interest</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <form class="row g-3" id="form-edit" enctype="multipart/form-data" action="#" method="POST">
                        ...
                        <div class="form-group pb-2">
                            <label for="name" class="pb-1">Interest Name</label>
                            <input type="text" id="nameEdit" name="name" class="form-control" required />
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-danger" data-bs-dismiss="modal">Close</button>
                            <button type="submit" class="btn btn-success" id="edit_interest_btn">Submit</button>
                        </div>
                    </form>
                </div>
            </div>
    ...
    <script>
        $(function() {
            ...
            $("#form-add").submit(function(e) {
                ...
                $("#add_interest_btn").text('Adding ...');
                $.ajax({
                    url: '{{ route('save.interest') }}',
                    method: 'POST',
                    data: dataForm,
                    cache: false,
                    contentType: false,
            ...
            // edit ajax request
            $(document).on('click', '.editInterest', function(e) {
                e.preventDefault();
                let id = $(this).attr('id');
                $.ajax({
                    url: '{{ route('edit.interest') }}',
                    method: 'GET',
                    data: {
                        id: id,
                        _token: '{{ csrf_token() }}'
            ...
            $("#form-edit").submit(function(e) {
                ...
                $("#edit_interest_btn").text('Updating ...');
                $.ajax({
                    url: '{{ route('update.interest') }}',
                    method: 'POST',
                    data: dataForm,
                    cache: false,
                    contentType: false,
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            swal({
                                title: "Updated!",
                                text: "interest has been updated!",
                                icon: "success",
                                button: "Close",
                            });
                            fetch();
